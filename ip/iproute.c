...
int print_route(const struct sockaddr_nl *who, struct nlmsghdr *n, void *arg)
{
	...
	if (r->rtm_ad) {
		SPRINT_BUF(b1);
		fprintf(fp, "ad %d ", (int)r->rtm_ad);
		/*fprintf(fp, "ad %s ", rtnl_dsfield_n2a(r->rtm_ad, b1, sizeof(b1)));*/
	}

    if (r->rtm_protocol_k)
    {
		/*SPRINT_BUF(b1);*/
        /*fprintf(fp, "AODV %s ",rtnl_dsfield_n2a(r->rtm_ad, b1, sizeof(b1)));*/
		fprintf(fp, "Protocol %d ", (int)r->rtm_protocol_k);
    }
#endif
	...
	if (!(r->rtm_flags&RTM_F_CLONED)) {
        ...
        {
            /*printf("this is the route out put\n");*/
...
#ifdef CNGC_NET/*c4istarlab*/
				fprintf(fp, " ad %d", nh->rtnh_ad);
				fprintf(fp, " Protocol %d", nh->rtnh_protocol);
#endif
			}
			...
			int *argcp, char ***argvp)
{
	...
	while (++argv, --argc > 0) {
		if (strcmp(*argv, "via") == 0) {
            ...
            /*printf("this is the protocol Aodv\n");*/
            /*req.r.rtm_padol_k = *argv;*/
            /*memcpy(req.r.rtm_protocol_k,*argv,length);*/
            /*req.r.rtm_protocol_k = AODV;*/
            /*__u8 protocol_k;*/
            unsigned protocol_k;
			NEXT_ARG();
            if(get_unsigned(&protocol_k, *argv, 0) || protocol_k == 0 ||  protocol_k > 256)
            {
                invarg("\"protocol_k,\" value is invalid\n", *argv);

            }
			/*if (rtnl_dsfield_a2n(&protocol_k, *argv))*/
				/*invarg("\"protocol_k,\" value is invalid\n", *argv);*/
            rtnh->rtnh_protocol = protocol_k;
            ...
            /*}*/
			if (get_unsigned(&ad, *argv, 0) || ad == 0 || ad > 256)
				invarg("\"ad\" value is invalid\n", *argv);
            rtnh->rtnh_ad = ad;
#endif
#if 0/*c4istarlab*/
        } else if(strcmp(*argv,"Aodv")==0){
            printf("this is the protocol Aodv\n");
            inet_prefix addr;
            unsigned ad;
            /*s_addattr(&req.r, RTA_PROTOCOL, *argv, sizeof(struct in_addr));*/
            /*if(c4istar_send(&rth,&req.n,0,0,NULL)<0){*/
                /*exit(2);*/
            /*}*/
			NEXT_ARG();
			if (get_unsigned(&ad, *argv, 0) || ad == 0 || ad > 256)
				invarg("\"ad\" is invalid\n", *argv);
			rtnh->rtnh_ad = ad - 1;
            rtnh->rtnh_protocol = *(argv-1);
			/*get_addr(&addr, *argv, req.r.rtm_family);*/
			/*if (req.r.rtm_family == AF_UNSPEC)*/
            /*{*/
				/*req.r.rtm_family = addr.family;*/
            /*}*/
			/*addattr_l(&req.n, sizeof(req), RTA_AODV, &addr.data, addr.bytelen);*/
            /*if(**argv>'0'&&**argv<'9'){*/
                /*printf("this is the administrative distance %d\n",**argv);*/
                /*s_addattr(&req.r, RTA_AD, *argv, sizeof(struct in_addr));*/
                /*if(c4istar_send(&rth,&req.n,0,0,NULL)<0){*/
                    /*exit(2);*/
                /*}*/
                /*return 0;*/
...
int s_addattr(struct nlmsghdr *n, int type, void *data, int alen)
{
    struct rtattr *attr;
...
int c4istar_send(struct rtnl_handle *rtnl, struct nlmsghdr *n,pid_t peer,
...
static int iproute_modify(int cmd, unsigned flags, int argc, char **argv)
{
	...
	while (argc > 0) {
        ...
        /*}else if(strcmp(*argv,"aodv") ==0 ){*/
			...
			if (rtnl_dsfield_a2n(&protocol_k, *argv))
				invarg("\"protocol_k,\" value is invalid\n", *argv);
            req.r.rtm_protocol_k = protocol_k;
#endif
